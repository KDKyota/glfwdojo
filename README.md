...
- スキニングとアニメーション
- 階段を含む地形と接地
- キャラクターの移動（三人称カメラの `W` / `A` / `S` / `D` はここで繋ぐ）
- 三人称追従カメラ（クォータニオン） — 完了
...
カメラには**自由視点**と**三人称追従**の2モードがあり、`F` で切り替えます。三人称ではキャラクター（`CesiumMan`）を注視点として周回し、位置は指数補間、向きは slerp で追従します。追従先のモデルが読み込まれていない場合は切り替わりません。
...
| 右ドラッグ     | 視点の回転（三人称では注視点まわりの周回）      |
| マウスホイール | ズーム（三人称では注視点までの距離）            |
