...
### HDR実装後の自然な発展として Bloom が未実装

HDR + Tone Mappingまで実装できたので、フレームバッファには1.0を超える輝度値が保持されています。この輝度情報を活かして、明るい部分だけを抽出してぼかし加算する **Bloom**（LearnOpenGLの次章に相当）を実装すると、光源やハイライトがより印象的に見えるようになります。
