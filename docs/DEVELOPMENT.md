...
15. [GPU デバッグ（RenderDoc / KHR_debug）](#gpu-デバッグrenderdoc--khr_debug)
16. [画面が真っ黒・真っ白になったときの調べ方](#画面が真っ黒真っ白になったときの調べ方)
17. [よくある落とし穴](#よくある落とし穴)
18. [カメラ](#カメラ)
19. [衝突判定](#衝突判定)
20. [モデル読み込みと単位系](#モデル読み込みと単位系)
21. [フレームアリーナと計測](#フレームアリーナと計測)
22. [Bloom の Compute Shader 化](#bloom-の-compute-shader-化)
23. [今後の課題](#今後の課題)
...
`renderShadowPasses()` が渡す VAO は毎回違います。

| 描画 | VAO | 有効な location | 宣言に対して欠けるもの |
| --- | --- | --- | --- |
| `renderFloor` | `planeVAO_` | 0,1,2 | 5,6,7 |
...
`GLFW_OPENGL_DEBUG_CONTEXT` を Debug ビルドでのみ要求**しています（`Window.cpp`）。
...
### `else` に波括弧を付け忘れると、無関係な debugMode まで巻き込んで壊れる

**症状:** 新しい `debugMode` を1つ足しただけなのに、`0: Normal lighting` の通常表示まで
おかしくなる。エラーは出ない。

**原因:** [`Shader::checkCompileErrors()`](../src/gl/Shader.cpp) はコンパイル・リンクに
失敗しても標準出力に出すだけで、例外を投げません。アプリはそのまま起動し続けます。

```glsl
} else                                   // ← { がない
    float visibility = sdfLightVisibility(...);
    FragColor = vec4(vec3(visibility), 1.0);   // else の外なので visibility が未定義
```

GLSL が `else` の本体とみなすのは次の1文だけです。インデントは揃っていても、
2行目は `else` の外に出ているため `visibility` は未定義変数の参照になり、
**そのシェーダープログラム全体のリンクが失敗**します。1つの `debugMode` にしか
手を入れていなくても、同じプログラムを共有する `debugMode == 0` まで巻き込まれます。

**対処:** コンソールに出る `ERROR::SHADER_COMPILATION_ERROR` /
`ERROR::PROGRAM_LINKING_ERROR` を必ず確認してから、症状の出ているコードを疑うこと。
「新しく足した箇所と無関係な場所まで同時に壊れた」は、
このプロジェクトではリンク失敗を最初に疑うサインです。
...
[`Scene::renderShadowPasses()`](../src/render/Scene.cpp) 側で
...
の頂点は `z = ±floorHalfExtent` にあります。一方 [`Scene::initUBO()`](../src/render/Scene.cpp)
...
auto sorted = updateTransparentInstances(); // 戻り値は ArraySpan（借り物）
```
...
- **リセットはフレーム末尾ではなく先頭。** `sorted` は Forward パスまで持ち回るので、
  末尾でリセットすると使用中のデータを無効化しうる
...
- 透過窓は**乗算パスと加算パスの2回**描いている（`Scene::renderTransparentWindows()`）
...
| 場所 | ビット | 次の読み方 |
| --- | --- | --- |
| ループ前 | `SHADER_IMAGE_ACCESS` | FBO へ書かれた `brightColorBuffer_` を `imageLoad` |
| ループ内 | `SHADER_IMAGE_ACCESS` | 前パスの `imageStore` を `imageLoad` |
| ループ後 | **`TEXTURE_FETCH`** | `renderToScreen` が `sampler2D` で読む |
